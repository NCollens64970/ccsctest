...
async function buttonHandler() {
  ...
  if (!validateRoomNumbers(building, classNumbers)) return;
...
async function validateRoomNumbers(building, enteredClassrooms) {
  await retrieveClassrooms(building).then((classes) => {
...
async function retrieveClassrooms(building) {
  return fetch("./classrooms.json")
    ...
    }) //take json portion of fetch
    .then((data) => {
      return data[building];
    }) //return building key of json
...
async function getCoordiantes(building, roomNumber) {
  return fetch("./coordinates.json")
    .then((response) => {
      return response.json();
    }) //Get json from coordinates.json
    .then((data) => {
      return data[building];
    }) //Get json from building selected
    .then((buildingJson) => {
      return buildingJson[String(roomNumber)];
    }) //Narrow to roomNumber (0 for bulding itself)
